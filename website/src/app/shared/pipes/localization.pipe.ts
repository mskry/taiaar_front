...
export class LocalizationPipe implements PipeTransform, OnDestroy {
  ...
  constructor(private util: UtilityService) {
    ...
    this.localization.recommended_supplier = `Recommended ${this.transform('suppliers', 0, langData.id != 14 ? 0 : 1)} For You`;
    this.localization.top_suppliers_near_you = `Top ${this.transform('suppliers', 0, langData.id != 14 ? 0 : 1)} Near You`;
    this.localization.top_suppliers =  `Top ${this.transform('suppliers', 0, langData.id != 14 ? 0 : 1)}`;
    this.localization.popular_brands = `Popular ${this.transform('brands', 0, langData.id != 14 ? 0 : 1)}`;
    this.localization.popular_products = `Popular ${this.transform('products', 0, langData.id != 14 ? 0 : 1)}`;
    this.localization.recently_viewed_products = `Recently Viewed ${this.terminology['english'].products}`;
    this.localization.unserviceable = this.terminology['english'].unserviceable;
