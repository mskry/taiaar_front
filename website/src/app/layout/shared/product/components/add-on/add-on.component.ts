...
export class AddOnComponent implements OnInit {
  ...
  calculatePrice() {
    ...
    console.log('this.customizeItems', this.customizeItems, this.addOnItems);
    ...
    this.customizeItems.forEach(item => {
      item.value.forEach(type => {
        ...
        // this.bottlePrice += Number(extract_num);
      });
    });
  }
  ...
  */
  ...
  getCustomDetail(customize: any, flag?:boolean): string {
    let str = "";
    let types = [];
    let bottle_price = 0;
    customize.data.forEach(i => {
      ...
      const bottleCount = i.value
      .map(el => el.bottle_count)
      .toString()
      .replace(/,/g, ", ");
      types.push(`${i.name}: ${typeNames}`);
      bottle_price += Number(bottleCount) * 0.10;
  ...
  getCustomBottlePriceDetail(customize: any) {
    ...
    bottle_price += Number(bottleCount) * 0.10 * customize.quantity;
    console.log(bottle_price);
