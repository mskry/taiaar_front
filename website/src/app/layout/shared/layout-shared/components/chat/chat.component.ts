...
export class ChatComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  ngOnInit() {
    ...
    if(this.openModal) {
      document.getElementById('openChatModal').click()
    }
  ...
  onOpen() { 
    // if(this.loggedIn) {
    //   this.socketService.connectListner()
    // }
    if (this.receiverType == 4) {
      this.getMessageId();
    } else if (this.message_id){
      this.fetchConversation();
      // this.getMessage();
      // this.getSendMessage();
    } else {
      setTimeout(() => {
        this.getSupplierMessageId()
      }, 2000);
    }
  ...
  joinRoom() {
    let r = Math.random().toString(36).substring(7) + Math.random().toString(36).substring(7) + Math.random().toString(36).substring(7);
    console.log("random", r);
    let payload = {
      message_id: this.message_id ? this.message_id : '',
      username: this.userService.currentUserValue.firstname
    };
    this.socketService.joinRoom(payload, this.supplier_id);
         setTimeout(() => {
    if(this.supplier_id) {
      this.getSupplierMessageId()
    } else {
  ...
  getSupplierMessageId() {
    let data = this.socketService.getMessageId()
    if (data && data.supplierId == this.supplier_id) {
